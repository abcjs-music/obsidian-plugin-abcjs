import { AbcVisualParams, MidiBuffer, SynthObjectController, TuneObject, renderAbc, synth } from "abcjs"
...
export class PlaybackElement extends MarkdownRenderChild {
  ...
  private readonly synthCtrl: SynthObjectController = new synth.SynthController()
  ...
  onload() {
    const { parsedOptions, source } = this.parseOptionsAndSource(this.markdownSource)
    const renderResp = renderAbc(this.el, source, Object.assign({}, DEFAULT_OPTIONS, parsedOptions))[0]
  ...
  parseOptionsAndSource(markdownSource: string): {
    parsedOptions: AbcVisualParams | undefined
    source: string
  } {
    ...
    return optionsMatch?.groups
      ...
      : { parsedOptions: undefined, source: markdownSource }
  }

  parseUserOptions(options: string): AbcVisualParams | undefined {
    try {
      return JSON.parse(options)
    } catch (e: unknown) {
      this.renderError(`Failed to parse user-options: ${e instanceof Error ? e.message : JSON.stringify(e)}`)
      return undefined
    }
  }
  ...
  private togglePlayback(): void {
    ...
    const isPlaying = this.midiBuffer.getIsRunning()
